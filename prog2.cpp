...
bool handleSTOI(const std::string str, int &num)
{
    ...
    {
        std::cerr << "Invalid argument: " << e.what()<<" "<<"on"<<str<< std::endl;
        return false;
    }
...
bool parseTuple(const std::string &tupleStr, Color &color)
{
    ...
    int bred = handleSTOI(tupleStr.substr(start, stop), rgbValues[0]);
    start = stop;
    stop = tupleStr.find(',',stop+1);
    ...
    if (bred && bgreen && bblue)
    {
        for (int val : rgbValues)
        {
            if (val > 255)
            {
                std::cout << "Color value of " << val << " is greater than 255\n";
    ...
    std::cout << "Valid tuple (" << rgbValues[0] << "," << rgbValues[1] << "," << rgbValues[2] << "),"
              << " found\n";
    return true;
}
...
int main(int, char**)
{
    ...
    // Our state
        ...
        std::array<std::array<Color, numRows>, numCols> grid;
    ...
    if (stream.is_open())
    {
        ...
        while (tokenStart != std::string::npos)
        {
            ...
            if (tagEnd > tagStart && tagStart != std::string::npos)
            {
                ...
                {
                    int row = count / numCols;
                    int col = count % numCols;
...
#endif
    {
        ...
        for(int row=0;row<numRows;row++)
        {
            ...
            ImVec4 colorVec=ImVec4(color.red/255,color.green/255,color.blue/255,200);
            ImGui::ColorButton("color",colorVec,0,ImVec2(30,30));
            ImGui::SameLine();
                }
            ImGui::NewLine();
            }
